...
class BattleRecordForm(forms.Form):
    ...
    record_date = forms.DateTimeField(label="时间", widget=forms.DateTimeInput(attrs={'type':'datetime-local'}, format='%Y-%m-%d %H:%M'), input_formats='%Y-%m-%d %H:%M')
    final_kill = forms.BooleanField(label="尾刀", label_suffix="", required=False, initial=False)
    comp_flag = forms.BooleanField(label="补偿刀", label_suffix="", required=False, initial=False)